...
class ActionSystem(private val service: AutomationService) {
    ...
    // Use java.util.Random for nextGaussian()
    ...
    private fun performClick(targetPoint: Point, params: JSONObject?, label: String) {
        ...
        for (i in 0 until repeat) {
            ...
            if (i > 0) {
                pX += (random.nextFloat() - 0.5f) * 5 // +/- 2.5 pixels
                pY += (random.nextFloat() - 0.5f) * 5
            }
    ...
    private fun performSwipe(startPoint: Point, params: JSONObject?, label: String) {
        ...
        val duration = params?.optLong("duration") ?: (300L + random.nextInt(300)) // 300-600ms
        ...
        // For simple cardinal directions:
        // UP/DOWN (Vertical) -> Offset X
        // LEFT/RIGHT (Horizontal) -> Offset Y
    ...
    private fun calculateTargetPoint(region: JSONObject, nodeRes: JSONObject?): Point {
        ...
        // 99.7% of clicks fall within +/- 3 sigma.
        // We want the clicks to mostly fall within the button.
        // Let's set 3 sigma = width / 2 (Radius).
        // So sigma = width / 6.
        // This ensures the bell curve fits nicely inside the button.
        ...
        // --- Safety Clamp ---
        // Even with Gaussian, there is a theoretical chance to be far out.
        // Or if the button is very small.
        // We clamp strictly to 90% of the button size to be safe.
        val safeW = width * 0.45 // From center, so 45% each side = 90% total
